...
def train_total(model, params, data, train_mask, val_mask, test_mask=None):
    torch.save(model, f"grad_grkan/node_classification_clean/models_saves/{params['dataset']}_{params['architecture']}_{params['conv_type']}")
    ...
    for epoch in range(params['epochs']):
        ...
        if should_stop:
            break
    print("load")
    model = torch.load(f"grad_grkan/node_classification_clean/models_saves/{params['dataset']}_{params['architecture']}_{params['conv_type']}")
    train_acc = efficient_evaluation_accuracy(data.y, out, train_mask)
    val_acc = efficient_evaluation_accuracy(data.y, out, val_mask)
    test_acc = efficient_evaluation_accuracy(data.y, out, test_mask)
    val_loss = efficient_evaluation_loss(data.y, out, val_mask, criterion)
...
def run_experiment(params, data_name):
    ...
    print(params)
    log_file = f"grad_grkan/node_classification_clean/logs/{data_name}_{params['architecture']}_{params['conv_type']}"
